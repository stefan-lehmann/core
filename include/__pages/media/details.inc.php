...
if (!OOMedia::isValid($media)) {
	cjoUrl::redirectBE(array('subpage'=>'media', 'media_category'=>$media_category));
}
...
$details .=
   '<span title="'.cjoI18N::translate('label_createdate').'"><img src="img/mini_icons/time.png" alt="" /> '.$media->getCreateDate(cjoI18N::translate("datetimeformat")).'</span>
...
$fields['title'] = new textField('title', cjoI18N::translate('label_title'), array('class' => 'large_item'));
...
$fields['category_cont'] = new readOnlyField('category_cont',cjoI18N::translate('label_file_category'));
$fields['category_cont']->addAttribute('style', 'float: left');
$fields['category_cont']->setValue(cjoSelectMediaCat::$sel_media->get(false));
...
foreach(cjoProp::getClangs() as $clang_id=>$name){
	...
	$fields['description_'.$name] = new textAreaField('description['.$clang_id.']', $flag.' '.cjoI18N::translate("label_description"));
	...
	if(!cjoProp::getUser()->hasClangPerm($clang_id)){
		...
		$fields['description_'.$name]->addAttribute('title', cjoI18N::translate("msg_no_permissions"));
	}
}

$fields['copyright'] = new textField('copyright', cjoI18N::translate('label_copyright'));
...
$upload_msg = '<span class="warning" style="margin-left: 200px!important; width:334px;">'.cjoI18N::translate('msg_install_or_upgrade_flash', '10').'</span>';

$fields['update_file'] = new textField('upload', cjoI18N::translate('label_update_file'), array ('style'=> 'width:250px;float:left;margin-right:4px'), 'update_file');
...
$fields['update_file2']->addOption(cjoI18N::translate('label_select_uploaded_file'), '');
...
//Add Fields:
$section = new cjoFormSection(TBL_FILES, cjoI18N::translate("title_details"), array ('file_id' => $oid), array('100%', '100%'));
...
if ($media_perm['w'] && $form->validate()) {
	...
	if (cjo_post('cjoform_save_button', 'bool')) {
        cjoUrl::redirectBE(array('subpage'=>'media','media_category'=>$media_category,'oid' => ''));
	}
	else {
		cjoUrl::redirectBE(array('subpage'=>$subpage,'media_category'=>$media_category,'oid'=>$oid));
	}
}
