...
if ($mode == 'user' && ($function == 'add' || $function == 'edit')) {
    ...
    if ($function == "add") {
        $fields['login'] = new textField('login', cjoI18N::translate('label_login'), array('style' => 'font-weight: bold'));
        $fields['login']->addValidator('notEmpty', cjoI18N::translate('msg_login_notEmpty'));
        $fields['login']->addValidator('isNot', cjoI18N::translate('msg_login_inUse'),$used_inputs['login'],true);
    }
    else{
    ...
    $fields['new_psw'] = new passwordField('new_psw', cjoI18N::translate('label_password'));
    $fields['new_psw']->addValidator('isLength', cjoI18N::translate('msg_new_psw_toShort'), array('min'=> 6));
    $fields['new_psw']->setHelp(cjoI18N::translate("note_psw_encrypted"));
    $fields['new_psw']->activateSave(false);

    $fields['status'] = new selectField('status', cjoI18N::translate('label_status'));
    $fields['status']->addAttribute('size', '1');
    $fields['status']->addOption(cjoI18N::translate("label_status_do_true"), '1');
    $fields['status']->addOption(cjoI18N::translate("label_status_do_false"), '0');

    $fields['perm_admin'] = new checkboxField('perm_admin', '&nbsp;');
    $fields['perm_admin']->setUncheckedValue();
    $fields['perm_admin']->addBox(cjoI18N::translate('label_editor_admin'), 'perm_admin');
    ...
    for ($i=0;$i<$sql->getRows();$i++) {
        ...
        $fields['perm_rights'] = new readOnlyField('perm_rights', cjoI18N::translate("label_group"));
    ...
    $fields['perm_lang'] = new readOnlyField('perm_lang', cjoI18N::translate("label_lang_perm"));
   ...
   if ($function == 'edit') {
     ...
     */
    $section = new cjoFormSection(TBL_USER, cjoI18N::translate('label_'.$function.'_user'), array ('user_id' => $oid));
        ...
        if (!$status) {
            ...
            if (cjoMessage::hasSuccesses()) {
                ...
                if (cjo_post('cjoform_save_button','boolean')) {
        		      cjoUrl::redirectBE(array('mode'=>'', 'function'=>'', 'oid'=>'', 'msg'=> $msg));
        		}
            }
        }
    }
...
$cols['icon']->addCondition('admin', '1', '<img src="img/silk_icons/user_orange.png" alt="" title="'.cjoI18N::translate('label_editor_admin').'" />');
...
$cols['login'] = new resultColumn('login', cjoI18N::translate("label_login"));
...
$cols['group'] = new resultColumn('description', cjoI18N::translate("label_group"), 'replace_array', array($replace_groups,'%s', 'delimiter_in' => '|','delimiter_out' => ', ' ));

$cols['langs'] = new resultColumn('langs', cjoI18N::translate("label_lang_perm"), 'call_user_func', array('cjoAssistance::convertToFlags', array('%s')));
...
$cols['login_tries'] = new resultColumn('login_tries', cjoI18N::translate("label_logintries"));
$cols['login_tries']->setHeadAttributes('class="icon"');
$cols['login_tries']->setBodyAttributes('class="icon"');

$cols['login_tries']->addCondition('login_tries', array('<',cjoProp::get('MAXLOGINS')), '<span title="'.cjoI18N::translate("label_reset_login_tries").'">%s</span>', array ('function' => 'reset_tries', 'mode' => 'user', 'oid' => '%user_id%'));
$cols['login_tries']->addCondition('login_tries', array('>=',cjoProp::get('MAXLOGINS')), '<b style="color:red" title="'.cjoI18N::translate("label_reset_login_tries").'">%s</b>', array ('function' => 'reset_tries', 'mode' => 'user', 'oid' => '%user_id%'));
...
$aktiv = '<img src="img/silk_icons/key2.png" title="'.cjoI18N::translate("label_status_do_false").'" alt="'.cjoI18N::translate("label_status_true").'" />';
$inaktiv = '<img src="img/silk_icons/key2_off.png" title="'.cjoI18N::translate("label_status_do_false").'" alt="'.cjoI18N::translate("label_status_false").'" />';
$disabled = '<img src="img/silk_icons/key2_start.png" title="" alt="'.cjoI18N::translate("label_admin").'" />';
