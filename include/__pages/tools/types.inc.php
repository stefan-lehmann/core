    ...
    if ($oid != '') {
        ...
        if (!cjoMessage::hasErrors()) {
            ...
            if ($sql->statusQuery("DELETE FROM ".TBL_ARTICLES_TYPE." WHERE type_id = '".$oid."' LIMIT 1",
                              cjoI18N::translate("msg_article_type_deleted")) &&
...
if ($function == "add" || $function == "edit" ) {
    ...
    //Fields
    $fields['name'] = new textField('name', cjoI18N::translate("label_type_name"));
    $fields['name']->addValidator('notEmpty', cjoI18N::translate("msg_type_name_notEmpty"), false, false);

    $fields['description'] = new textAreaField('description', cjoI18N::translate("label_type_description"), array('rows' => 2));
    $fields['description']->addValidator('notEmpty', cjoI18N::translate("msg_type_description_notEmpty"), false, false);
     ...
     */
    $section = new cjoFormSection(TBL_ARTICLES_TYPE, cjoI18N::translate('label_'.$function."_type"), array ('type_id' => $oid));
    ...
    if ($form->validate()) {
        ...
        if (cjo_post('cjoform_save_button', 'boolean')) {
    		if ($function == 'edit') {
    		    cjoMessage::addSuccess(cjoI18N::translate("msg_article_type_updated"));
    		}
    		else {
    		    cjoMessage::addSuccess(cjoI18N::translate("msg_article_type_saved"));
    		}
    	}
    	if ($function == "add") {
