...
if ($function == "delete"){
	...
	if($oid == 0){
		cjoMessage::addError(cjoI18N::translate("msg_langs_0_not_deleted"));
	}
	else {
	    if (cjoGenerate::deleteCLang($oid)){
	      ...
	      unset($oid);
	        ...
	        cjoMessage::addError(cjoI18N::translate("msg_langs_not_deleted"));
	    }
	}
}
...
if ($function == "add" || $function == "edit" ) {
    ...
    if ($function == 'add') {
		...
		$oid = '';
        $fields['new_id'] = new selectField('new_id', cjoI18N::translate("label_id"));
    ...
    $fields['name']->addValidator('notEmpty', cjoI18N::translate("msg_clang_name_notEmpty"), false, false);
    $fields['name']->activateSave(false);

    $fields['iso'] = new selectField('iso', cjoI18N::translate("label_iso"));
    $fields['iso']->addAttribute('size', '1');
    $fields['iso']->addValidator('isLength', cjoI18N::translate("msg_iso_not"), array('min' => 2, 'max' => 2), false);
    ...
    elseif (count($CJO['CLANG']) > 1) {
        ...
        $fields['headline1']->setValue(cjoI18N::translate("label_sync_clang"));
        $fields['headline1']->needFullColumn(true);
        
        $fields['sync_clang'] = new checkboxField('sync_clang', '&nbsp;',  array('style' => 'width: auto;'),'sync_clang');
        $fields['sync_clang']->addBox(cjoI18N::translate("label_sync_clang_enable"), '1');  
        $fields['sync_clang']->activateSave(false);
        
        $fields['sync_master'] = new selectField('sync_master', cjoI18N::translate("label_sync_master"));
        ...
        $fields['prior'] = new checkboxField('prior', cjoI18N::translate("label_update_sync"),  array('style' => 'width: auto;'));
        $fields['prior']->addBox(cjoI18N::translate("label_update_prior"), '1');  
        ...
        if ($CJO['TEASER_ENABLED'] == true) {
            ...
            $fields['teaser']->addBox(cjoI18N::translate("label_update_teaser"), '1');  
        ...
        $fields['navi_item']->addBox(cjoI18N::translate("label_update_navi_item"), '1');  
        ...
        $fields['status']->addBox(cjoI18N::translate("label_update_status"), '1');  
        ...
        if ($CJO['ONLINE_FROM_TO_ENABLED'] == true) {        
            ...
            $fields['online_from_to']->addBox(cjoI18N::translate("label_update_online_from_to"), '1'); 
        ...
        if ($CJO['LOGIN_ENABLED'] == true) {
            ...
            $fields['type_id']->addBox(cjoI18N::translate("label_update_type_id"), '1'); 
        ...
        if (cjoAddon::isActivated('comments')) {
            ...
            $fields['comments']->addBox(cjoI18N::translate("label_update_comments"), '1'); 
        ...
        $fields['cat_group']->addBox(cjoI18N::translate("label_update_cat_group"), '1');  
        ...
        $fields['redirect']->addBox(cjoI18N::translate("label_update_redirect"), '1'); 
        ...
        $fields['admin_only']->addBox(cjoI18N::translate("label_update_admin_only"), '1'); 
        ...
        $fields['check_all']->addBox('<b>'.cjoI18N::translate('label_select_deselect_all').'</b>', '1'); 
     ...
     */
    $section = new cjoFormSection(TBL_CLANGS, cjoI18N::translate("label_".$function."_clang"), array ('id' => $oid));
    ...
    if ($form->validate()) {
        ...
        if ($function == "edit") {
           ...
           cjoProp::saveFromFile(cjoPath::pageConfig('clangs'));
...
if (!$function) {
    ...
    $cols['iso'] = new resultColumn('iso', cjoI18N::translate("label_iso").' ','sprintf', '<img src="img/flags/%s.png" alt="" />');
