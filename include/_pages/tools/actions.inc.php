...
if ($function == 'delete'){
    ...
    if($oid != ''){
        ...
        if (!empty($result)) {
            ...
            cjoMessage::addError($I18N->msg("msg_action_cannot_be_deleted",$result[0]['name']).'<br/>'.implode(' | ',$temp));
        }
...
if ($function == "add" || $function == "edit" ) {
    ...
    $fields['sadd']->addBox('ADD ('.$I18N->msg("label_action_add").')', '1');
    ...
    $fields['sedit']->addBox('EDIT ('.$I18N->msg("label_action_edit").')', '1');
    ...
    $fields['sdelete']->addBox('DELETE ('.$I18N->msg("label_action_delete").')', '1');
     ...
     */
    $section = new cjoFormSection(TBL_ACTIONS, $I18N->msg("label_".$function."_action"), array ('id' => $oid));
    ...
    if ($form->validate() || $add_no_user) {
        ...
        if (cjo_post('cjoform_save_button', 'boolean')) {
            if ($function == 'edit') {
                cjoMessage::addSuccess($I18N->msg("msg_action_updated"));
            }
            else {
                cjoMessage::addSuccess($I18N->msg("msg_action_added"));
            }
            unset($function);
        }
    }
