...
class SmartyValidate {
    ...
    public static function connect(&$smarty, $reset = false) {
        ...
        if (SmartyValidate::is_valid_smarty_object($smarty)) {
            ...
            throw new cjoException("SmartyValidate: [connect] I need a valid Smarty object.");
            return false;
        }
    }
    ...
    public static function disconnect() {
        ...
        unset($SMARTY_VALIDATE);
        if (!isset ($_dummy)) $_dummy = '';
    ...
    public static function register_form($form, $reset = false) {
        ...
        if (SmartyValidate::is_registered_form($form) && !$reset) {
            cjoValidateEngine::remember($form);
            return false;
        } else {
            $SMARTY_VALIDATE[$form] = array();
            $SMARTY_VALIDATE[$form]['registered_funcs']['criteria'] = array();
            $SMARTY_VALIDATE[$form]['registered_funcs']['transform'] = array();
            $SMARTY_VALIDATE[$form]['validators'] = array();
            $SMARTY_VALIDATE[$form]['is_error'] = false;
            $SMARTY_VALIDATE[$form]['is_init'] = true;  
            cjoValidateEngine::remember($form);
    ...
    public static function is_registered_form($form = SMARTY_VALIDATE_DEFAULT_FORM) {
        global $SMARTY_VALIDATE;
        return isset($SMARTY_VALIDATE[$form]);
    }
    ...
    public static function _failed_fields(&$formvars, $form = SMARTY_VALIDATE_DEFAULT_FORM, $revalidate = false) {
        ...
        $_ret = array();
        $GLOBALS['SmartyValidate'][$form] = array();
        
        $_sess = & $SMARTY_VALIDATE[$form]['validators'];

        foreach($_sess as $_key => $_val) {

            if (isset($SMARTY_VALIDATE[$form]['page']) && 
                $_sess[$_key]['page'] != $SMARTY_VALIDATE[$form]['page']) {
            ...
            if (is_array($_ret) && in_array($_full_field, $_ret)) continue;
            
            $GLOBALS['SmartyValidate'][$form][$_key] = $_field;
    ...
    public static function is_valid(&$formvars, $form = SMARTY_VALIDATE_DEFAULT_FORM) {
        ...
        // Show Messages
        if (cjo_post('cjo_form_name','string') != $form) {
            $_is_valid[$form] = false;
        }

        if (isset($_is_valid[$form])) {
            // already validated the form
            //self::add_form_to_session($form);     
        ...
        if (!SmartyValidate::is_valid_smarty_object($_smarty_obj)) {
            throw new cjoException("SmartyValidate: [is_valid] No valid smarty object, call connect() first.");
            return false;
        }

        if (!SmartyValidate::is_registered_form($form)) {
            throw new cjoException("SmartyValidate: [is_valid] form '$form' is not registered.");
            return false;
        } elseif (count($SMARTY_VALIDATE[$form]['validators']) == 0) {
            // nothing to validate
            //self::add_form_to_session($form);
            return true;
        }
...
//    private function add_form_to_session($form) {
//        global $SMARTY_VALIDATE;
//  
//        $SESS_SMARTY_VALIDATE = cjo_session('SMARTY_VALIDATE', 'array', array());
//        $SESS_SMARTY_VALIDATE[$form] = $SMARTY_VALIDATE[$form];
//        cjo_set_session('SMARTY_VALIDATE', $SESS_SMARTY_VALIDATE);  
//    }
    /**
    ...
    public function register_object($object_name, &$object) {
        if (!is_object($object)) {
            throw new cjoException("SmartyValidate: [register_object] not a valid object.");
            return false;
        }
     ...
     * @param string $func_name the function being registered
     */
    public function register_criteria($name, $func_name, $form = SMARTY_VALIDATE_DEFAULT_FORM) {
    ...
    public static function is_registered_criteria($name, $form = SMARTY_VALIDATE_DEFAULT_FORM) {
        if (!SmartyValidate::is_registered_form($form)) {
            throw new cjoException("SmartyValidate: [is_registered_criteria] form '$form' is not registered.");
            return false;
        }
        return isset($SMARTY_VALIDATE[$form]['registered_funcs']['criteria'][$name]);
    }
    ...
    public static function is_registered_transform($name, $form = SMARTY_VALIDATE_DEFAULT_FORM) {
        ...
        if (!SmartyValidate::is_registered_form($form)) {
            throw new cjoException("SmartyValidate: [is_registered_transform] form '$form' is not registered.");
            return false;
        }
        return isset($SMARTY_VALIDATE[$form]['registered_funcs']['transform'][$name]);
    }
    ...
    public static function register_validator($id, $field, $criteria, $empty = false, $halt = false, $transform = null, $form = SMARTY_VALIDATE_DEFAULT_FORM) {
        ...
        if (!SmartyValidate::is_registered_form($form)) {
            throw new cjoException("SmartyValidate: [register_validator] form '$form' is not registered.");
            return false;
        }
        ...
        $SMARTY_VALIDATE[$form]['validators'][]  = $_validator;
    }
    ...
    public static function set_transform($id, $transform, $form = SMARTY_VALIDATE_DEFAULT_FORM) {
        ...
        if (($_validator_key = SmartyValidate::is_registered_validator($id,$form)) === false) {
            throw new cjoException("SmartyValidate: [set_transform] validator '$i$CJOnot registered.");
            return false;
        }

        $SMARTY_VALIDATE[$form]['validators'][$_validator_key]['transform'] = $transform;
    }
    ...
    public static function is_registered_validator($id, $form = SMARTY_VALIDATE_DEFAULT_FORM) {
        ...
        if (!SmartyValidate::is_registered_form($form)) {
            throw new cjoException("SmartyValidate: [is_registered_validator] form '$form' is not registered.");
            return false;
        }

        foreach($SMARTY_VALIDATE[$form]['validators'] as $_key => $_val) {
            if ($SMARTY_VALIDATE[$form]['validators'][$_key]['id'] == $id) {
    ...
    public static function unregister_validator($id, $form = SMARTY_VALIDATE_DEFAULT_FORM) {
        ...
        if (!SmartyValidate::is_registered_form($form))  return false; 

        foreach($SMARTY_VALIDATE[$form]['validators'] as $_key => $_val) {
            if (isset($SMARTY_VALIDATE[$form]['validators'][$_key]['id'])
                    && $SMARTY_VALIDATE[$form]['validators'][$_key]['id'] == $id) {
               unset($SMARTY_VALIDATE[$form]['validators'][$_key]);
               break;
            }
        }
    ...
    public static function set_page($page, $form = SMARTY_VALIDATE_DEFAULT_FORM) {
        ...
        $SMARTY_VALIDATE[$form]['page'] = $page;
        $SMARTY_VALIDATE[$form]['is_error'] = false;
        $SMARTY_VALIDATE[$form]['is_init'] = true;
    }
    ...
    public static function _execute_transform($name, $value, $params, &$formvars, $form) {
        ...
        if (SmartyValidate::is_registered_transform($name, $form)) {
            ...
            if (!function_exists($_func_name)) {
                ...
                if ($_plugin_file = $_smarty_obj->_get_plugin_filepath('validate_transform', $name)) {
                    ...
                    throw new cjoException("SmartyValidate: [is_valid] transform function '$name' was not found.");
                    return false;
                }
            }
        }
        if (strpos($_func_name,'->') !== false) {
            ...
            if (!method_exists($_object, $_method_name)) {
                throw new cjoException("SmartyValidate: [is_valid] method '$_method_name' is not valid for object '$_object_name'.");
                return false;
            }
    ...
    public static function _register_function($type, $name, $func_name, $form = SMARTY_VALIDATE_DEFAULT_FORM) {
        ...
        if (!SmartyValidate::is_registered_form($form)) {
            throw new cjoException("SmartyValidate: [register_$type] form '$form' is not registered.");
            return false;
        }
        if (strpos($func_name,'->') !== false) {
            ...
            $_object = SmartyValidate::_object_instance($_object_name, $_dummy);
            if (!method_exists($_object, $_method_name)) {
                throw new cjoException("SmartyValidate: [register_$type] method '$_method_name' is not valid for object '$_object_name'.");
                return false;
            }
            ...
            if (!is_callable(array($_match[1], $_match[2]))) {
                throw new cjoException("SmartyValidate: [register_$type] static method '$func_name' does not exist.");
                return false;
            }
        } elseif (!function_exists($func_name)) {
            throw new cjoException("SmartyValidate: [register_$type] function '$func_name' does not exist.");
            return false;
        }
        $SMARTY_VALIDATE[$form]['registered_funcs'][$type][$name] = $func_name;
        return true;
    }
    ...
    public static function _get_registered_func_name($type,$name,$form) {
        ...
        return isset($SMARTY_VALIDATE[$form]['registered_funcs'][$type][$name])
           ? $SMARTY_VALIDATE[$form]['registered_funcs'][$type][$name]
           : false;
    }
        ...
        if (SmartyValidate::is_registered_criteria($criteria,$form)) {
            ...
            if (!function_exists($_func_name)) {
                ...
                if ($_plugin_file = $_smarty_obj->_get_plugin_filepath('validate_criteria', $criteria)) {
                    ...
                    throw new cjoException("SmartyValidate: [is_valid] criteria function '$criteria' was not found.");
                    return false;
                }
            }
        }
    ...
    public static function _smarty_assign($vars = array()) {
        ...
        if (!is_object($_smarty_obj)) {
            throw new cjoException("SmartyValidate: [assign] no valid smarty object found, call connect() first.");
            return false;
        }
        ...
        foreach($SMARTY_VALIDATE as $_key => $_val) {
            $_info[$_key]['is_error'] = isset($SMARTY_VALIDATE[$_key]['is_error']) ? $SMARTY_VALIDATE[$_key]['is_error'] : null;
        }
