 ...
 */
class cjoAutoload {
    static protected $registered = false, $cacheFile = null, $cacheChanged = false, $reloaded = false, $dirs = array(), $addedDirs = array(), $classes = array();
    ...
    static public function register() {
        ...
        if (false === spl_autoload_register(array(__CLASS__, 'autoload'))) {
            throw new cjoException(sprintf('Unable to register %s::autoload as an autoloading method.', __CLASS__));
        }

        self::$cacheFile = cjoPath::cache('autoload.cache');
        self::loadCache();
    ...
    static private function loadCache() {
        ...
        if (!self::$cacheFile || !is_readable(self::$cacheFile)) return;
    ...
    static public function saveCache() {
        ...
        if (self::$cacheChanged) {
            if (!file_exists(self::$cacheFile) || is_writable(dirname(self::$cacheFile))) {
                file_put_contents(self::$cacheFile, cjo_json_encode_utf8(array(self::$classes, self::$dirs)));
    ...
    static public function removeCache() {
        cjoFile::deleteFile(self::$cacheFile);
    }
    ...
    static private function _addDirectory($dir) {
        ...
        if ($files = glob($dir . '*.inc.php', GLOB_NOSORT)) {
            foreach ($files as $file) {
                self::addFile($file);
            }
        }

        if ($subdirs = glob($dir.'*', GLOB_ONLYDIR | GLOB_NOSORT)) {
