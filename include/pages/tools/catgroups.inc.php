...
if ($function == "delete") {
    ...
    if ($oid == '1') {
        cjoMessage::addError($I18N->msg("msg_catgroup_1_not_deleted"));
    }
    else {
		$sql = new cjoSql();
		$qry = "SELECT name, id
        ...
        if (!empty($temp))
            cjoMessage::addError($I18N->msg("msg_catgroup_still_used").'<br/>'.implode(' | ',$temp));

        if (!cjoMessage::hasErrors()) {
            $sql->flush();
            $sql->statusQuery("DELETE FROM ".TBL_ARTICLES_CAT_GROUPS." WHERE group_id = '".$oid."' LIMIT 1",
                              $I18N->msg("msg_catgroup_deleted"));
...
if ($function == "add" || $function == "edit" ) {
    ...
    if ($function == 'add') {
        ...
        $oid = '';
        $fields['group_id'] = new selectField('group_id', $I18N->msg("label_id"));
    ...
    $fields['group_name'] = new textField('group_name', $I18N->msg("label_catgroup_name"));
    $fields['group_name']->addValidator('notEmpty', $I18N->msg("msg_cat_group_name_notEmpty"), false, false);

    $fields['group_structure'] = new textField('group_structure', $I18N->msg("label_catgroup_structure"));
    $fields['group_structure']->addValidator('notEmpty', $I18N->msg("msg_catgroup_structure_notEmpty"), false, false);

    $fields['group_style'] = new colorpickerField('group_style', $I18N->msg("label_catgroup_style"));
    $fields['group_style']->addValidator('notEmpty', $I18N->msg("msg_catgroup_style_notEmpty"), false, false);
     ...
     */
    $section = new cjoFormSection(TBL_ARTICLES_CAT_GROUPS, $I18N->msg("label_".$function."_catgroup"), array ('group_id' => $oid));
    ...
    if ($form->validate()) {
        ...
        if (cjo_post('cjoform_save_button', 'boolean')) {
            if ($function == 'edit') {
                cjoMessage::addSuccess($I18N->msg("msg_catgroup_updated"));
            }
            else {
                cjoMessage::addSuccess($I18N->msg("msg_catgroup_saved"));
            }
        }
    }
}
...
$cols['group_id'] = new resultColumn('group_id', $I18N->msg("label_id"));
$cols['group_id']->setHeadAttributes('class="icon"');
$cols['group_id']->setBodyAttributes('class="icon"');
$cols['group_id']->delOption(OPT_SORT);

$cols['group_name'] = new resultColumn('group_name', $I18N->msg("label_catgroup_name").' ');
$cols['group_name']->delOption(OPT_SORT);

$cols['group_structure'] = new resultColumn('group_structure', $I18N->msg("label_catgroup_structure").' ');
$cols['group_structure']->delOption(OPT_SORT);

$cols['group_style'] = new resultColumn('group_style', $I18N->msg("label_catgroup_style").' ', 'sprintf', '<span style="border: 1px solid #666; background-color: %1$s; display: block; width: 2em; height: 1em;" title="%1$s"></span>');
