...
if (cjo_post('cjoform_cancel_button','boolean')) {
	cjoUrl::redirectBE(array('media_category'=>$media_category));
}
...
if ($function == 'add' || $function == 'edit') {
	...
	$fields['comment'] = new textField('comment', cjoI18N::translate('label_category_comment'));
	...
	if ($function == 'add') {
		...
		$title = cjoI18N::translate("title_add_media_category");
	}
	else {
		...
		$fields['headline1'] = new headlineField(cjoI18N::translate("label_info"), slide);
	...
	if ($form->validate()) {
       ...
       cjoUrl::redirectBE(array('media_category'=>$media_category,'function' => '','msg'=>'msg_data_saved'));
	}	
}
    ...
    $cols['thumb'] = new resultColumn('thumb', cjoI18N::translate('label_name'), 'sprintf', '<span>%s</span>');
    ...
    $cols['file_num'] = new resultColumn('file_num', null, 'sprintf', '<span title="'.cjoI18N::translate('label_file_num').'"><img src="img/mini_icons/page_white_acrobat.png" alt="" />%s '.cjoI18N::translate('label_media').'</span>');
    ...
    $cols['comment'] = new resultColumn('comment', cjoI18N::translate('label_category_comment'));
    $cols['comment']->setBodyAttributes('style="width: 45%"');
    // Bearbeiten link
    $img = '<img src="img/silk_icons/page_white_edit.png" title="'.cjoI18N::translate("label_edit_category").'" alt="'.cjoI18N::translate("label_edit").'" />';
    ...
    $cond['delete'] = '<img src="img/silk_icons/bin.png" title="'.cjoI18N::translate("label_delete_category").'" alt="'.cjoI18N::translate("label_delete_category").'" />';
    $cols['delete'] = new staticColumn($cond['delete'], null);
    $cols['delete']->setBodyAttributes('width="60"');
    $cols['delete']->setBodyAttributes('class="cjo_delete"');

    $rowAttributes = ' onclick="location.href=\''.cjoUrl::createBEUrl(array('media_category'=>$parent_id, 'function'=> '', 'msg'=>'msg_data_saved')).'\';" ' .
...
function cjoFormateCells($cell){
    ...
    switch($cell['name']){
    	...
    	case 'thumb':
    			$icon = (isset($cell['row']['children']['unformated']) && $cell['row']['children']['unformated'] > 0) ? 'folders' : 'folder';
    			$icon = (isset($cell['row']['children']['unformated']) && $cell['row']['file_num']['unformated'] > 0) ? $icon.'2' : $icon;
		...
		case 'name':
				...
				$curr_cell .= '<ul class="col_details">'.
							  ...
							  '	<li class="quicklinks">'.
				              '		<ul>'.
							  '		<li class="first">'.cjoUrl::createBELink(cjoI18N::translate('title_media'), array('subpage' => 'media', 'media_category'=>$media_category), array(), 'title="'.cjoI18N::translate("label_display_category_media").'" class="first"').'</li>'.
							  '		<li>'.cjoUrl::createBELink(cjoI18N::translate('title_addmedia'), array('subpage' => 'addmedia', 'media_category'=>$media_category), array(), 'title="'.cjoI18N::translate("title_addmedia").'"').'</li>'.
							  '		</ul>'.
							  ' </li>'.
							  '</ul>';
				break;
