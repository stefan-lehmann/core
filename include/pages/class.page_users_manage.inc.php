...
class cjoPageUsersManage extends cjoPage {
        
    function __construct() {
        parent::__construct();
        new cjoPageUsersManageGroups();
        new cjoPageUsersManageUsers();
    }
    ...
    public static function updateSatus($id, $mode, $status) {
        ...
        $update->setWhere("user_id='".$id."' AND login NOT REGEXP '^group_'");
        $update->setValue("status",$status);
        $update->addGlobalUpdateFields();
        if ($update->Update(cjoI18N::translate("msg_user_status_updated"))) {
            cjoExtension::registerExtensionPoint('USER_UPDATED', array('ACTION'  => 'STATUS',
                                                                       'user_id' => $id,
                                                                       'status'  => $status));
        }
    }
...
if ($mode == 'user' && $function == 'delete' && $oid != '') {
    ...
    if (cjoProp::getUser()->getValue("user_id")!=$oid) {
        $sql->flush();
        ...
        if ($sql->statusQuery("DELETE FROM ".TBL_USER." WHERE login NOT REGEXP '^group_' AND user_id = '".$oid."' LIMIT 1",
                          cjoI18N::translate("msg_editor_deleted"))) {
            cjoExtension::registerExtensionPoint('USER_DELETED', $results[0]);
        }           
    }
    else{
        cjoMessage::addError(cjoI18N::translate("msg_editor_notdeleteself"));
    }
    unset($results);
    unset($function);
    unset($mode);
...
if ($mode == 'user' && $function == 'reset_tries' && $oid != '') {
    ...
    $update->addGlobalUpdateFields();
    if ($update->Update(cjoI18N::translate("msg_login_tries_reseted"))) {
        cjoExtension::registerExtensionPoint('USER_UPDATED', array('ACTION' => strtoupper($function),
                                                                   'user_id' =>  $oid));
    }
    unset($function);
...
if ($mode == 'groups' && $function == 'delete' && $oid != '') {
    ...
    if ($sql->getRows() == 0) {
        ...
        if ($sql->statusQuery("DELETE FROM ".TBL_USER." WHERE login REGEXP '^group_' AND user_id = '$oid' LIMIT 1",
                          cjoI18N::translate("msg_group_deleted"))) {
            cjoExtension::registerExtensionPoint('USER_GROUP_DELETED', $results[0]);  
        }                         
    }
    else{
        cjoMessage::addError(cjoI18N::translate("msg_group_has_children"));
    }
    unset($function);
    unset($mode);
}
...
if (!$mode || $mode == 'groups') {
	require_once cjoPath::inc('pages/users/manage_groups.inc.php');
}
if (!$mode || $mode == 'user') {
	require_once cjoPath::inc('pages/users/manage_user.inc.php');
}