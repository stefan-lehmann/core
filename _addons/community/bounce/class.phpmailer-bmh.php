...
class BounceMailHandler {
    ...
    function processBounce($pos,$type,$totalFetched) {
        ...
        if ($result['rule_no'] == '0000') {
            $result = bmhSubjectRules($subject, $this->debug_body_rule);
        }
        ...
        if ((empty($email) || preg_match('/[A-Z0-9._%-]+@[A-Z0-9.-]+\.[A-Z]{2,6}/i',$email, $matches))
            && preg_match('/\b[A-Z0-9._%-]+@[A-Z0-9.-]+\.[A-Z]{2,6}\b/i',$header->fromaddress,$matches)) {
            $email = $matches[0];
        }

        if ((empty($email) || preg_match('/[A-Z0-9._%-]+@[A-Z0-9.-]+\.[A-Z]{2,6}/i',$email, $matches))
            && preg_match('/\b[A-Z0-9._%-]+@[A-Z0-9.-]+\.[A-Z]{2,6}\b/i',$body,$matches)) {
            $email = $matches[0];
        }
    ...
    function mailbox_exist($mailbox,$create=true) {
        if (trim($mailbox) == '' || !strstr($mailbox,'INBOX.') ) {
            ...
            $this->error_msg = "Invalid mailbox name for move operation. Cannot continue.<br />\n".
                         "TIP: the mailbox you want to move the message to must include 'INBOX.' at the start.<br />\n";
            $this->output();
            return false;
        }
        $port = $this->port . '/' . $this->service . '/' . $this->service_option;
