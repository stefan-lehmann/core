        ...
        if (is_file($file) && is_readable($file)) {
            ...
            if (is_array($sqlsplit)) {
                foreach ($sqlsplit as $qry) {
					...
					preg_match('/(?<=%)TBL[A-Z0-9_]*(?=%)/', $qry['query'], $tbl_name);

                	if ($tbl_name[0] != '' && defined($tbl_name[0])){
						eval('$replace_name = '.$tbl_name[0].';');
						$qry['query'] = str_replace('%'.$tbl_name[0].'%', $replace_name, $qry['query']);
                	}
