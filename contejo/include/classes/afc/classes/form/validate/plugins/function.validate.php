...
function smarty_function_validate($params, &$smarty) {
    ...
    if (isset($_sess['validators']) && is_array($_sess['validators'])) {
        ...
        if (isset($GLOBALS['SmartyValidate'][$_form]) && is_array($GLOBALS['SmartyValidate'][$_form])) {

            if ($GLOBALS['SmartyValidate'][$_form][$_validator_key] == '') return;

            foreach( $GLOBALS['SmartyValidate'][$_form] as $key=>$field){
                if ($field == $_sess['validators'][$_validator_key]['field']){
                    if ($key > $_validator_key) return;
                }
            }
        }
