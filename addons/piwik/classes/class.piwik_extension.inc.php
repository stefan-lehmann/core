...
class cjoPiwikExtension {
    ...
    public static function replaceVars($params) {
        ...
        if (strpos($content,'PIWIK_NL_TRACKING_PIXEL[]') !== false) {    	
    	    $content = str_replace('PIWIK_NL_TRACKING_PIXEL[]', self::generateNewsletterPixel(), $content);
    	}  	
    	return $content;
    }
    ...
    public static function generatePiwikCode($action_name='') {
    	...
    	$custom_url = false;
    	if (cjo_get('html5video','bool') && cjoAddon::isAvailable('html5video')) {
    	...
    	$piwik_url = preg_replace('/^https?:\/\//','',cjoAddon::getParameter('URL', self::$addon).'/');
        ...
        return '<!-- Piwik -->'.
        	   ...
        	   'if (window.location.protocol != undefined && window.location.protocol.match(/^http/)) {'.             
               '    try {'.
               '        var piwikTracker = Piwik.getTracker(pkBaseURL + "piwik.php", '.cjoAddon::getParameter('IDSITE', self::$addon).');'.
               '        piwikTracker.setDownloadExtensions("'.cjoAddon::getParameter('TRACK_AS_DOWNLOADS', self::$addon).'" );'.
               '        piwikTracker.setDownloadClasses("'.cjoAddon::getParameter('DOWNLOAD_CLASS', self::$addon).'" );'.        
    ...
    public static function generateNewsletterPixel($goal=1) {
        ...
        $article_id = cjoProp::get('ARTICLE_ID');
    	$article    = OOArticle::getArticleById($article_id);

    	if (!OOArticle::isValid($article)) return false;
    	
    	$url = cjoUrl::setServerUri(false, false).cjoUrl::setServerPath();
        ...
        return sprintf('<img src="%scjo_piwik/%s/%s/%%user_id%%/%s/%s" width="1" height="1" alt="" border="0">',
		                ...
		                $article_id,
		                (int) $goal,		                
                        md5($ip.cjoProp::getInstname()),
                        cjoAddon::getParameter('EMAIL_CAMPAIGN_FILENAME', self::$addon));
    }
    ...
    public static function setSessionTrackRequest() {
        ...
        $article_id = cjoProp::get('ARTICLE_ID');
        $clang = cjoProp::get('CUR_CLANG');
        
        $url = parse_url(cjoUrl::getUrl($article_id));
        ...
        cjoUrl::redirectFE($article_id, $clang, $params, $anchor);
    }
    ...
    public static function redirectEmailPixelTracking() {
        ...
        $dirname = preg_replace('/^.*'.cjoAddon::getParameter('redirect_path', self::$addon).'\//','',$path['dirname']);
        $get     = explode('/',$dirname);
        $params  = array();
        ...
        if (!OOArticle::isValid($article) ||
            $path['basename'] != cjoAddon::getParameter('EMAIL_CAMPAIGN_FILENAME', self::$addon) || 
            $get[3] != md5(cjo_server('REMOTE_ADDR','string').cjoProp::get('INSTNAME'))) return false;
            
        cjoProp::set('ARTICLE_ID',$article->getId());
    ...
    private static function generateUrl($get_params=array(), $separator=false) {
    	...
    	$set_params['idsite']          = !empty($get_params['idsite'])
    	                               ? $get_params['idsite'] 
    	                               : (string) cjoAddon::getParameter('IDSITE', self::$addon);
    	...
    	$set_params['url']             = !empty($get_params['url'])
    	                               ? $get_params['url'] 
    	                               : cjo_server('REQUEST_URI','string', cjoUrl::getUrl(cjoProp::get('ARTICLE_ID')));
        ...
        $url = cjoAddon::getParameter('URL', self::$addon).'/piwik.php?'.$query;
    	return preg_replace('/[^:]\/{2,}/','/', $url);
    }

    public static function testRedirectPath() {

        if (cjoProp::isBackend() || strpos(cjo_server('REQUEST_URI', 'string'), '/'.cjoAddon::getParameter('EMAIL_CAMPAIGN_FILENAME', self::$addon).'/') === false) return;
    
        if (strpos(cjo_server('REQUEST_URI', 'string'), cjoAddon::getParameter('redirect_path', self::$addon)) !== false) {
