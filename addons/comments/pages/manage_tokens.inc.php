...
if ($function == 'to_spam' ||
	...
	switch($function){
		case 'to_spam' :
			...
			cjoMessage::addSuccess($I18N_7->msg("msg_token_made_spam", cjo_get('token','string')));
			break;
		case 'to_ham' :
			$b8->unlearn(cjo_get('token','string'), 'spam');
			$b8->learn(cjo_get('token','string'), 'ham');
			cjoMessage::addSuccess($I18N_7->msg("msg_token_made_ham", cjo_get('token','string')));
			break;
		default:
			$b8->unlearn(cjo_get('token','string'), 'spam');
			$b8->unlearn(cjo_get('token','string'), 'ham');
			cjoMessage::addSuccess($I18N_7->msg("msg_token_unlearned", cjo_get('token','string')));
	}
    unset($function);
}
...
$cols['token'] = new resultColumn('token', $I18N_7->msg('label_token'));

$ham = '<img src="img/silk_icons/accept.png" title="'.$I18N_7->msg("label_to_spam").'" alt="'.$I18N->msg("label_ham").'" />';
$spam = '<img src="img/silk_icons/exclamation.png" title="'.$I18N_7->msg("label_to_ham").'" alt="'.$I18N->msg("label_spam").'" />';
$cols['count'] = new resultColumn('count', $I18N_7->msg('label_status'));
