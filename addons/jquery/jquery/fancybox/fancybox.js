...
if (mobile) {
	document.writeln('<script type="text/javascript" src="'+JQUERY_ADDON_PATH+'/_code.photoswipe-3.0.4.min.js"></script>');
}
...
$(function() {
	...
	$("a[rel^=imagebox]").each(function() {

		var rel = $(this).attr('rel');
		if (imageboxes[rel] == 1) return true;
		imageboxes[rel] = 1;
		...
		$("a[rel=" + rel + "]")
				.fancybox({ 'padding' : 0,
							...
							'titleFormat' : function(title,currentArray, currentIndex, currentOpts) {
								if (title == '') {
									currentOpts.orig.parents()
									.each( function() {
												var $this = $(this);
												var headline = $this.siblings('h1, h2, h3, h4,h5');
												if (headline.length > 0) {
													var temp = headline.eq(0).clone();
								...
								return '<span id="fancybox-title-over" style="display:none"><b>'
										+ title
										+ '</b>'
							...
							'onComplete' : function() {
								...
								$("#fancybox-wrap")
									.hover(function(){window.clearTimeout(timeout);
	...
	$("a[rel^=flashbox]").each(function() {

		var $this = $(this);
		var href = $this.attr('href');
		var url = href.replace(/\?.*/, '');
		var width = href.replace(/.*w=(\d+).*/, '$1') * 1;
		var height = href.replace(/.*h=(\d+).*/, '$1') * 1;
	...
	$("a[rel^=videobox]").each(function() {
		...
		var $this = $(this);
		var href = $this.attr('href');
		var width = href.replace(/.*width=(\d+).*/, '$1') * 1;
		var height = href.replace(/.*height=(\d+).*/, '$1') * 1;
	...
	$("a[rel^=fancybox]").each(function() {
		...
		if ((isNaN(width) || width < 10) && (isNaN(height) || height < 10)) {
			var settings = {
				'padding' : 0,
				'titleShow' : false,
				'autoScale' : true,
