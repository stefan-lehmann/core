...
class liveEdit {
    ...
    private static $addon = 'developer';
    private $livePath;
    private $ModulePath;
    private $ModuleInputExtension;
    private $ModuleOutputExtension;
    private $TemplatePath;
    private $TemplateExtension;
    private $TmplExtension;

    public function __construct(){
        
        $this->livePath               = self::getEditPath(); 
        $this->ModulePath             = $this->livePath.'/'.cjoAddon::getParameter('MODULE_PATH', self::$addon).'/';
        $this->TemplatePath           = $this->livePath.'/'.cjoAddon::getParameter('TEMPLATE_PATH', self::$addon).'/';
        $this->ModuleInputExtension   = '.'.cjoAddon::getParameter('MODULE_INPUT_EXTENSION', self::$addon);
        $this->ModuleOutputExtension  = '.'.cjoAddon::getParameter('MODULE_OUTPUT_EXTENSION', self::$addon);
        $this->TemplateExtension      = '.'.cjoAddon::getParameter('TEMPLATE_EXTENSION', self::$addon);
        $this->TmplExtension          = cjoAddon::getParameter('TMPL_EXTENSION', self::$addon);
    }

    public function getModuleFiles(){

        if ($handle = opendir($this->ModulePath)) {
        ...
        if (is_array($modulesDB)){
            foreach($modulesDB as $key=>$var){
                if (!@in_array($key,$modulesFiles) || $overwrite==true){
                    $inputFilename  = $this->ModulePath.$key.$this->ModuleInputExtension;
                    $outputFilename = $this->ModulePath.$key.$this->ModuleOutputExtension;
        ...
        if(is_array($templatesDB)){
            foreach($templatesDB as $key=>$var){
                ...
                if(!@in_array($key,$templateFiles) || $overwrite==true){
                    $templateFilename = $this->TemplatePath.$key.$this->TemplateExtension;
    ...
    public function removeTemplateFiles(){
        ...
        if (is_array($templateFiles)){
            foreach($templateFiles as $template_id){
                unlink($this->TemplatePath.$template_id.$this->TemplateExtension);
            }
        }
    }
    ...
    public function syncModules(){
        ...
        if (is_array($module_files)) {
            foreach($module_files as $module_id){
                ...
                $input_path  = $this->ModulePath.$module_id.$this->ModuleInputExtension;
                $output_path = $this->ModulePath.$module_id.$this->ModuleOutputExtension;
        ...
        if (cjoProp::isBackend() && cjoProp::getUser() && self::md5ModulLayouts()){
    ...
    public function syncTemplates(){
        ...
        if (is_array($template_files)){
            foreach($template_files as $template_id){
                ...
                $template_path = $this->TemplatePath.$template_id.$this->TemplateExtension;
                ...
                if (file_exists($template_path)){
                    ...
                    if ($db_template_modified > $template_modified || empty($db_template_by_id[$template_id])){
                        ...
                        @unlink($template_path);
                        $templatePathDeveloper = $this->TemplatePath.$template_id.$this->TemplateExtension;
                        $templatePathContejo = cjoPath::generated('templates', $template_id.'.template');

                        if (file_exists($templatePathContejo)){
                            copy($templatePathContejo,$templatePathDeveloper);
                            @chmod($templatePathDeveloper,cjoProp::getFilePerm());
                        }
                    }
                }
            }
    ...
    public function generateTemplate($template_id){
        ...
        @chmod($templatePathContejo, cjoProp::getFilePerm());
    }
    ...
    public function regenerateArticlesByJS($params) {
        ...
        if (cjoAddon::getParameter('regenerate', self::$addon)){
            $regenerateString = cjoAddon::getParameter('regenerate', self::$addon);
    ...
    private static function md5ModulLayouts() {
        ...
        $files = cjoAssistance::rglob($liveEdit->livePath.'/'.$liveEdit->TmplExtension, '*.'.$liveEdit->TmplExtension);
    ...
    public static function getTmplExtension() {
        return cjoAddon::getParameter('TMPL_EXTENSION', self::$addon);
    }
    
    public static function getEditPath($file='') {
        return cjoPath::absolute(cjoPath::page(cjoAddon::getParameter('EDIT_PATH', self::$addon).'/'.$file));
    }

    public static function insertCss($params) {
        $files = array();
        if ($handle = opendir(self::getEditPath('css'))) {
            while (false !== ($file = readdir($handle))) {
                if ($file != "." && $file != ".." && cjoFile::getExtension($file) == 'css') {
                    $file = cjoAddon::getParameter('EDIT_PATH', self::$addon).'/css/'.$file;
                    $files[] = cjoUrl::page($file);
                }
            }
            closedir($handle);
        }
        return cjo_insertCss($params['subject'], $files);
    }
    
    public static function initAddon() {
        
        if (cjoProp::isSetup() || cjo_request('function') == 'import') return;
    
        // Check for Modules Updates in Files
        if (cjoAddon::getParameter('modules', self::$addon, false)) {
        
            $liveEdit = new liveEdit();
            $articles_overwrite = $page == 'addon' && $addonname == self::$addon && $activate=='1' ? true : false;
            $liveEdit->writeModuleFiles($articles_overwrite);
        
            if (cjo_request('page') != 'developer' && cjo_request('action') != 'update') {
                
                if (cjoProp::isBackend()) {
                    if (cjoLogin::isBackendLogin()) {
                        cjoAddon::setParameter('regenerate', $liveEdit->syncModules(), self::$addon) ;
                    ...
                    cjoAddon::setParameter('regenerate', $liveEdit->syncModules(), self::$addon) ;
                    $liveEdit = new liveEdit();
                    $liveEdit->regenerateArticlesByModultypId(cjoAddon::getParameter('regenerate', self::$addon) );
                }
            }
        }
        
        // Check for Template Updates in Files
        if (cjoAddon::getParameter('templates', self::$addon, false)) {
        
            $liveEdit = new liveEdit();
            $templates_overwrite = $page == 'addon' && $addonname == self::$addon && $activate == '1' ? true : false;
            $liveEdit->writeTemplateFiles($templates_overwrite);
            $liveEdit->syncTemplates();
        }
        
        // Developer-CSS asu geschütztem Verzeichnis einbinden
        if (cjoProp::isBackend() && cjo_request('page', 'string') != 'setup') {
