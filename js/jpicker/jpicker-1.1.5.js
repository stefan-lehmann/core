      ...
      return this.each(
        function()
        {
          ...
          if (settings.window.expandable) {
        	  ...
        	  $this.wrap('<span class="jPicker"></span>');
            ...
            var h  = $this.height() *1;
            	h += $this.css('padding-top').replace(/px/,'') *1;
            	h += $this.css('padding-bottom').replace(/px/,'') *1;
            	h += $this.css('border-top-width').replace(/px/,'') *1;
            	h += $this.css('border-bottom-width').replace(/px/,'') *1;

            	$this.css({'width': ($this.width()*1-22)+'px', 'cursor' :'pointer'}).attr('readonly','readonly');
            
            $this.parent()
            	.append('<span class="Icon"><span class="Color">&nbsp;</span><span class="Alpha">&nbsp;</span><span class="Image" title="Click To Open Color Picker">&nbsp;</span><span class="Container">&nbsp;</span></span>');
          }
          ...
          var isLessThanIE7 = parseFloat(navigator.appVersion.split('MSIE')[1]) < 7 && document.body.filters, // needed to run the AlphaImageLoader function for IE6
            ...
            currentColorChanged =
              function(ui, context)
              {
                ...
                currentPreview.css({ backgroundColor: hex && '#' + hex || 'transparent',
                					 backgroundImage: hex && hex.length == 6 ? 'none' : 'url(' + images.clientPath + 'preview-opacity.png)'});
            ...
            expandableColorChanged =
              function(ui, context)
              {
                ...
                if (settings.window.bindToInput&&settings.window.updateInputColor) {
                  ...
                  var value = hex && hex.length <= 6 ? '#' + hex : '';
            ...
            show =
              function()
              {
                ...
                var top = win.position.y == 'top'    ? (win.input.offset().top +  25) + 'px' :
                    	  win.position.y == 'center' ? (win.input.offset().top - 156) + 'px' :
                          win.position.y == 'bottom' ? (win.input.offset().top - 312) + 'px' 
                        		  					 : (win.input.offset().top + parseInt(win.position.y)) + 'px';
            ...
            hide =
              function()
              {
            	 ...
            	 var value = settings.window.input.val().replace(/[^a-fA-F0-9]/g, '').toLowerCase().substring(0, 6);
      			 if (value != '') value = '#'+value;
                 settings.window.input.val(value); 
                  ...
                  container.css( // positions must be set and display set to absolute before source code injection or IE will size the container to fit the window
                    {
                      ...
                      position: 'absolute',
                      top: win.position.y == 'top' ? (popup.offset().top - 25) + 'px' :
                           win.position.y == 'center' ? (popup.offset().top - 156) + 'px' :
                           win.position.y == 'bottom' ? (popup.offset().top + 312) + 'px' : (popup.offset().top + parseInt(win.position.y)) + 'px'
                    });
                }
                else
                {
